...
const FridayAgent: React.FC<FridayAgentProps> = ({ userProfile }) => {
  ...
  // Refs mirror transcript state so WebSocket callbacks always read the latest value (fixes stale closure)
  const currentTranscriptRef = useRef('');
  const fridayTranscriptRef = useRef('');
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.serverContent?.inputTranscription) {
              ...
              if (text) {
                // Update both state (for UI) and ref (so turnComplete callback reads the latest value)
            ...
            if (message.serverContent?.turnComplete) {
               ...
               // Read from refs — state values are stale inside this closure
               const capturedUser = currentTranscriptRef.current;
               const capturedFriday = fridayTranscriptRef.current;
               setMessages(prev => [
                 ...prev,
                 { id: Date.now().toString(), role: 'user', text: capturedUser || "[Vocal Command]", timestamp: new Date() },
                 { id: (Date.now() + 1).toString(), role: 'friday', text: capturedFriday, timestamp: new Date() }
               ]);
               
               if (capturedUser) {
                 detectEmotion(capturedUser);
               }

               currentTranscriptRef.current = '';
               fridayTranscriptRef.current = '';
               setCurrentTranscript('');
               setFridayTranscript('');
            }
            if (message.serverContent?.outputTranscription) {
              const outText = message.serverContent.outputTranscription.text || '';
              setFridayTranscript(prev => {
                const next = prev + outText;
                fridayTranscriptRef.current = next;
                return next;
              });
